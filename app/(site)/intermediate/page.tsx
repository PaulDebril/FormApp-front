...
export default function Intermediate() {
  ...
  const handleAddCard = () => {
    ...
    setGroups(groups.map(group => {
      if (group.id === activeGroupId) {
        return {
          ...group,
          cards: [...group.cards, { id: group.cards.length + 1, name: newCard.name }],
        };
      }
      return group;
    }));
  ...
  const handleAddGroup = () => {
    ...
    setGroups([...groups, { id: groups.length + 1, name: newGroupName, cards: [] }]);
    setNewGroupName("");
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="min-h-[calc(100vh-57px-97px)] flex-1">
        <h3 className="text-xl font-bold text-center mt-8">Interventions intermédiaires</h3>

        <div className="mt-8 grid grid-cols-1 md:grid-cols-5 gap-4 px-10">
          {cards.map((card) => (
            <Card key={card.id} className="flex flex-col items-center justify-center p-4 text-center">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              ...
              <DialogDescription>
                Remplissez les informations ci-dessous pour ajouter une nouvelle intervention.
              </DialogDescription>
        ...
        <h3 className="text-xl font-bold text-center mt-12">Centre par intermédiaire</h3>

        <div className="mt-8 px-10">
          <Accordion type="single" collapsible defaultValue={`group-${groups[0]?.id}`}>
            {groups.map((group) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                      <DialogTrigger asChild>
                        ...
                        >
                          <Plus className="w-6 h-6 text-gray-500" />
                          <span className="text-gray-500 font-medium">Ajouter une carte</span>
                        </div>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogTitle>Ajouter une nouvelle intervention</DialogTitle>
                        <DialogDescription>
                          Remplissez les informations ci-dessous pour ajouter une nouvelle intervention dans {group.name}.
                        </DialogDescription>
